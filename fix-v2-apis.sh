...
# Script to fix syntax errors in v2 APIs by properly copying types from v1alpha1

set -e

# Function to properly fix a v2 API
fix_v2_api() {
    local resource=$1
    ...
    echo "Fixing v2 API for $resource..."
    ...
    # Create a clean v2 types.go by properly extracting from v1alpha1
    cat > "$v2_types" << 'EOF'
/*
...
EOF
    ...
    # Extract Parameters struct cleanly
...
EOF
    ...
    # Extract Observation struct if it exists, otherwise create a basic one
...
EOF
    ...
    # Add the rest of the v2 API structure
    cat >> "$v2_types" << EOF
...
EOF
    ...
    echo "  Fixed v2 API for $resource"
}
...
echo "Fixing v2 API syntax errors..."
