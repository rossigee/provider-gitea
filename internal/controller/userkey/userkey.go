...
const (
	...
	errGetUserKey        = "failed to get user key"
	errCreateUserKey     = "failed to create user key"
	errUpdateUserKey     = "failed to update user key"
	errDeleteUserKey     = "failed to delete user key"
...
func parseExternalName(externalID string) (username string, id int64, err error) {
...
func (e *externalClient) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	_, span := tracing.StartSpan(ctx, "userkey.observe",
		tracing.SpanAttrs("userkey", tracing.ResourceName(mg), "observe")...)
	defer span.End()
	...
	key, err := e.client.GetUserKey(ctx, username, id)
	if err != nil {
...
func (e *externalClient) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	_, span := tracing.StartSpan(ctx, "userkey.create",
		tracing.SpanAttrs("userkey", tracing.ResourceName(mg), "create")...)
	defer span.End()
	...
	key, err := e.client.CreateUserKey(ctx, fp.Username, &clients.CreateUserKeyRequest{
		Title: fp.Title,
		Key:   fp.Key,
	})
	if err != nil {
	...
	meta.SetExternalName(cr, fmt.Sprintf("%s/%d", fp.Username, key.ID))

	return managed.ExternalCreation{}, nil
}

func (e *externalClient) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	_, span := tracing.StartSpan(ctx, "userkey.update",
		tracing.SpanAttrs("userkey", tracing.ResourceName(mg), "update")...)
	defer span.End()
	...
	// Key material is immutable; only the title can be updated.
	title := cr.Spec.ForProvider.Title
	if _, err := e.client.UpdateUserKey(ctx, username, id, &clients.UpdateUserKeyRequest{
		Title: &title,
	}); err != nil {
...
func (e *externalClient) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	_, span := tracing.StartSpan(ctx, "userkey.delete",
		tracing.SpanAttrs("userkey", tracing.ResourceName(mg), "delete")...)
	defer span.End()
	...
	if _, err := e.client.GetUserKey(ctx, username, id); err == nil {
		if err := e.client.DeleteUserKey(ctx, username, id); err != nil {
