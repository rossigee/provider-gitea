...
import (
	...
	"github.com/rossigee/provider-gitea/apis/issue/v2"
	"github.com/rossigee/provider-gitea/internal/clients"
)

// fakeClient is a hand-rolled clients.Client stub exposing only the issue verbs
...
type fakeClient struct {
	clients.Client
	getIssue   *clients.Issue
	getErr     error
	created    *clients.CreateIssueOptions
	createResp *clients.Issue
	createErr  error
	updated    *clients.UpdateIssueOptions
	deleteErr  error
	deleted    bool
}

func (f *fakeClient) GetIssue(_ context.Context, _, _ string, _ int64) (*clients.Issue, error) {
	return f.getIssue, f.getErr
}

func (f *fakeClient) CreateIssue(_ context.Context, _, _ string, req *clients.CreateIssueOptions) (*clients.Issue, error) {
	f.created = req
	return f.createResp, f.createErr
}

func (f *fakeClient) UpdateIssue(_ context.Context, _, _ string, _ int64, req *clients.UpdateIssueOptions) (*clients.Issue, error) {
	f.updated = req
	return f.getIssue, nil
}

func (f *fakeClient) DeleteIssue(_ context.Context, _, _ string, _ int64) error {
	f.deleted = true
	return f.deleteErr
}

func newCR(externalName string) *v2.Issue {
	cr := &v2.Issue{}
	cr.SetName("my-issue")
	cr.Spec.ForProvider.Title = "my-issue"
...
func TestObserveNotFound(t *testing.T) {
	f := &fakeClient{getErr: errors.Wrap(clients.NewNotFoundError("issue", "acme/widgets#1"), "failed to get issue")}
	e := &external{client: f}
...
// TestObserveAvailableAndUpToDate: an existing, matching issue must be marked
// Available and up-to-date (lesson #2/#6).
func TestObserveAvailableAndUpToDate(t *testing.T) {
	f := &fakeClient{getIssue: &clients.Issue{ID: 7, Number: 1, Title: "my-issue", Body: "hello", State: "open"}}
	e := &external{client: f}
...
func TestObserveDriftStillAvailable(t *testing.T) {
	f := &fakeClient{getIssue: &clients.Issue{ID: 7, Number: 1, Title: "my-issue", Body: "old", State: "open"}}
	e := &external{client: f}
...
// authoritative issue number returned by the backend (lesson #3/#7).
func TestCreateSetsExternalName(t *testing.T) {
	f := &fakeClient{createResp: &clients.Issue{ID: 7, Number: 42, Title: "my-issue"}}
	e := &external{client: f}
...
// TestDeleteIdempotentOn404: a delete against an already-absent issue must
// succeed so the finalizer releases (lesson #16).
func TestDeleteIdempotentOn404(t *testing.T) {
	f := &fakeClient{deleteErr: clients.NewNotFoundError("issue", "acme/widgets#1")}
	e := &external{client: f}
	...
	if !f.deleted {
		t.Fatalf("expected DeleteIssue to have been called")
	}
}
